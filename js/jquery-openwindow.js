...
(function ($) { 
	$.fn.update_page = function(options){
		...
		var _this =  $(this);
	    var total_page = parseInt( settings.total_page );
	    var current_page = parseInt( settings.current_page );
 ...
 * 支付渠道控件
 * DEMO:
 * $("#payChlCode").payChlCodeMultiselect({selectedValues:["1003", "1006"]});
 */
(function ($) { 
	$.fn.payChlCodeMultiselect = function (options) { 
		var settings = {
				baseUrl: rootUrl,
				url: "/ajax/payChlCode.json?math="+Math.random(),
				multiple: true,
				filter: true,
	    		...
	    		_this.empty();
       			if(data.list){
       				...
       				for(i=0;i<obj.length;i++){
       					_html = "<option value='"+obj[i].payChlCode+"'";
       					$.each(settings.selectedValues, function(x, val){
       						if(obj[i].payChlCode == val){
       							_html += " selected=\"selected\" ";
       							return false;
       						}
       					});
       					_html += ">"+obj[i].payChlCode+":"+obj[i].payChlName+"</option>";
       					_this.append(_html);
       				}
       			}
 ...
 * bankCode控件
 * DEMO:
 * $("#bankCodeCode").fundMultiselect({selectedValues:["103", "104"]});
 */
(function ($) { 
	$.fn.bankCodeMultiselect = function (options) { 
		var settings = {
				baseUrl: rootUrl,
				url: "/ajax/bankCode.json?math="+Math.random(),
				multiple: true,
				filter: true,
				selectedValues:""//[aa,bb,cc]
			}; 
       		...
       		function(data){
	    		_this.empty();
       			if(data.list){
       				var obj = data.list;
       				for(i=0;i<obj.length;i++){
       					_html = "<option value='"+obj[i].bankCode+"'";
       					$.each(settings.selectedValues, function(x, val){
       						if(obj[i].bankCode == val){
       							_html += " selected=\"selected\" ";
       							return false;
       						}
       					});
       					_html += ">"+obj[i].bankCode+":"+obj[i].bankName+"</option>";
       					_this.append(_html);
       				}
       			}
       			_this = _this.multiselect({multiple:settings.multiple});
 ...
 * disCode控件
 * DEMO:
 * $("#disCode").fundMultiselect({selectedValues:["HB000A001"]});
 */
(function ($) { 
	$.fn.disCodeMultiselect = function (options) { 
		var settings = {
				baseUrl: rootUrl,
				url: "/ajax/disCode.json?math="+Math.random(),
				multiple: true,
				filter: true,
				selectedValues:""//[aa,bb,cc]
			}; 
       		...
       		function(data){
	    		_this.empty();
       			if(data.list){
       				var obj = data.list;
       				for(i=0;i<obj.length;i++){
       					_html = "<option value='"+obj[i].disCode+"'";
       					$.each(settings.selectedValues, function(x, val){
       						if(obj[i].disCode == val){
       							_html += " selected=\"selected\" ";
       							return false;
       						}
       					});
       					_html += ">"+obj[i].disCode+":"+obj[i].disName+"</option>";
       					_this.append(_html);
       				}
       			}
       			_this = _this.multiselect({multiple:settings.multiple});
...
(function ($) { 
	$.fn.openCustWindow = function (options) {
		var settings = {
			baseUrl: rootUrl,
			url: "/ajax/custno?callbackJsFunc=closeWindow", 
			width: 800, 
			height: 550,
			title:"客户基本信息查询"
		};
